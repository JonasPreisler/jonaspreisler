				...
				json.elements do
					json.array! [*1] do
						...
						json.buttons do
							json.array! [*1] do
								json.type "web_url"
								json.url "http://jones.webshop8.dk/shop/showbasket.html?BasketLink=1&AddMultiple=1&ProductID=|#{@link_to_shorten.product_id}&Amount=|1&VariantGroups=|#{'Farve' if @link_to_shorten.color?}#{'%3BStørrelse' if @link_to_shorten.size?}&Variants=|#{@link_to_shorten.color}#{'%3B' if @link_to_shorten.size?}#{@link_to_shorten.size}"
								json.title "Læg i kurv"
							end
						end
					end
